// The implementation is based on the implementation of Rabitqlib.
// https://github.com/VectorDB-NTU/RaBitQ-Library/blob/main/rabitqlib/fastscan/highacc_fastscan.hpp
...
// data order of packed quantization code, please refer to code and the link offered above for detailed information
 ...
 * @param quantization_code quantization code, stored as uint8
...
inline void pack_codes(
    ...
    for (size_t row = 0; row < num_rd; row += kBatchSize) {
        ...
        // i.e., we get the codes for 8 dims of 32 vectors and re-organize the data layout
...
// ============================================================================
// High-accuracy FastScan (from fastscan_highacc.hpp)
// ============================================================================

/**
 * @brief Change u16 lookup table to u8. Since we use more bits (higher accuracy)
 * to quantize data vector by rabitq+, we also need to increase the accuracy of data in
 * lut.
...
inline void accumulate_hacc(
    ...
    for (size_t m = 0; m < num_codebook; m += 4) {
        ...
        for (auto &i : accu) {
            __m512i lut = _mm512_load_si512(hc_lut);
